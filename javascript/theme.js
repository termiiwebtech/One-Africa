...
jQuery(function($) {
	...
	var mailURL = 'http://www.zmthemes.com/dropout-html/php/mail.php';
	var windowLocation = window.location.href;

	if( mailURL.indexOf('www') < 0 && windowLocation.indexOf('www') > -1 ) {
		mailURL = mailURL.replace('https://', 'https://www.').replace('http://', 'http://www.');
	} else if( mailURL.indexOf('www') > -1 && windowLocation.indexOf('www') < 0 ) {
		mailURL = mailURL.replace('www.', '');
	}
	...
	if( $('.gallery').length ) {
		...
		$('.gallery').each(function(index) {
			var id = 'gallery-' + galleryCount;
			$('.gallery').eq(index).attr('id', id);
